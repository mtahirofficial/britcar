...
const { shop, webhook, order, vendor, cutoff, orderitem, purchaseorder } = dbModels;

const db = {
    getSingleField: async (model, field, where) => {
        const result = await model.findOne({ attributes: [field], where });
    ...
    getWholeSingleColumn: async (model, field, where) => {
        const result = await model.findAll({ attributes: [field], where });
        const list = result.map(({ dataValues }) => { return dataValues[field] })
        return list ? list : '';
    },
    ...
    getData: async (model, where) => {
        ...
        const list = data.map(({ dataValues }) => { return dataValues })
        return list
    },
    updateData: async (model, data, where) => {
        const created = await model.update(data, { where })
        return created
    },
    ...
    getDataWithWhereAndInclude: async (model, include, where, dataOrder) => {
        const result = await model.findAll({ where, include, order: dataOrder });
        const data = result.map(({ dataValues }) => { return dataValues })
        return data
    },
    incrementValue: async (model, data, where) => {
        const result = await model.increment(data, { where });
        return result
    },
    // shop
    findShop: async domain => {
        const result = await shop.findAll({ where: { domain } });
        return result
    }
    , saveShop: async data => {
    ...
    , updateShop: async (data, domain) => {
        const dataSavedResponse = await shop.update(data, { where: { domain } });
        return dataSavedResponse;
    },
    getShopData: async shopName => {
        const data = await shop.findAll({ attributes: ['shop_currency', "service_id"], where: { shop_name: shopName } });
    ...
    getSinglePurchaseOrders: async (where) => {
        const purchaseOrders = await purchaseorder.findOne({ where })
        return purchaseOrders ?? {}
    },
    getPurchaseOrders: async (where, dataOrder, include) => {
        const purchaseOrders = await purchaseorder.findAll({ where, order: dataOrder, include })
        return purchaseOrders
    },

    // vendors
    getVendors: async where => {
        const dataSavedResponse = await vendor.findAll({ where, include: { model: cutoff } });
        return dataSavedResponse;
    }
    ...
    , updateVendor: async data => {
        const dbResponse = await vendor.update(data, { where: { id: data.id } })
        return dbResponse
    }
    ...
    , updateCutoff: async data => {
        const dbResponse = await cutoff.update(data, { where: { id: data.id } })
        return dbResponse
    }
    // webhooks
    ...
    , deleteWebhooks: async id => {
        const webhookResponse = await webhook.destroy({ where: { shopId: id } });
        return webhookResponse;
    }
}
module.exports = { db }