...
const ordersController = {
  getAlternateProducts: async (req, res) => {
    ...
    const moneyFormat = await db.getSingleField(shop, 'moneyFormat', { domain });
      ...
      if (edges.length > 0) {
        for (const { node } of edges) {
          ...
          if (variantId !== id) {
            ...
            productList.push({ label: `${currencySymbol}${variant.price} | ${variant.barcode} | ${variant.product.productType} | ${variant.product.vendor} | ${title}`, value: variant })
          }
        }
  ...
  saveOrderToDatabase: async (req, res) => {
    try {
      ...
      if (!domain) {
        return res.status(400).send({ status: 400, message: 'Missing shop domain header' });
      }
      ...
      if (!shopId || !accessToken) {
        return res.status(404).send({ status: 404, message: 'Shop not found or missing access token' });
      }
      const orderId = await db.getSingleField(order, 'orderId', { orderId: req.body.id });
      if (!orderId) {
        const orders = []
        const data = {
          shopId,
        ...
        if (savedOrder.orderId) {
          ...
          for (const lineItem of req.body.line_items) {
            ...
            if (products[lineItem.product_id] === undefined) {
              product = await cda.getProduct(domain, lineItem.product_id, accessToken);
            ...
            if (product.variants[lineItem.variant_id].image_id > 0) {
              image = product.images[product.variants[lineItem.variant_id].image_id]
            ...
            const dimensions = await cda.getDimensions(domain, accessToken, lineItem.product_id)
            ...
            const orderitem = {
              ...
              name: lineItem.variant_title !== '' ? "Pack of " + lineItem.variant_title + " - " + lineItem.title : lineItem.title,
              // name: lineItem.name,
          ...
          const POS = await cda.generatePos(orders, accessToken, shopId, domain)
        }
      }
  ...
  createPurchaseOrders: async (req, res) => {
    ...
    const where = {
      [Op.and]: [
        { active: true }, { shopId }, { financialStatus: 'paid' }
      ]
    }
    const activeOrders = await db.getOrders(where)
    console.log({ activeOrders });
    const POS = await cda.generatePos(activeOrders, accessToken, shopId, domain)
    res.send(POS)
  },
  getPurchaseOrders: async (req, res) => {
    ...
    const vendorWhere = { enabled: true, autoSubmit: true, email: { [Op.ne]: null } }

    const includeForOpen = [{ model: vendor, where: vendorWhere }]
    const whereForOpen = { shopId, status: 'null', submittedAt: null, timeUntilCutoff: { [Op.lt]: ct } }

    const openPos = await db.getPurchaseOrders(whereForOpen, orderBy, includeForOpen)

    for (const order of openPos) {
      try {
        const response = await cda.submitOrder(order.id)
        console.log("response 243", response);
      } catch (error) {
        console.log("error.message 244", error.message);
      }
    }
  ...
  submitOrder: async (req, res) => {
    const { id } = req.params
    let isSubmitted = await db.getSingleField(purchaseorder, 'submittedAt', { id });
    if (isSubmitted === null || isSubmitted === '') {
      const submittedAt = new Date()
      const submitted = await db.updateData(purchaseorder, { submittedAt, status: 'sub' }, { id })
      if (submitted[0]) {
  ...
  fetchPoItems: async (req, res) => {
    ...
    const pos = await db.getDataWithWhereAndInclude(
      purchaseorder, //model
      { model: purchaseorderitem, where: { received: { [Op.lt]: sequelize.col('qty') }, status: { [Op.ne]: 're' } } }, //include
      { [Op.and]: { vendorId, status: 'sub', shopId } }, //where
      [['id', 'DESC'], [purchaseorderitem, 'id', 'DESC']] //order
    )
    const vendorData = await db.getSingleRecord(vendor, { id: vendorId, shopId })
    for (const po of pos) {
  ...
  receivedProduct: async (req, res) => {
    ...
    if (response.hasOwnProperty('variant')) {
      ...
      const qtyUpdated = await cda.adjustQty(variant.inventory_item_id, qtyToShow, shopData)

      const orderId = await db.getSingleField(purchaseorderitem, 'orderId', { id: id })
      const DispatchBay = await cda.getDispatchBay(orderId, shopData)

      let dispatchBay = '', customer = {};
      if (DispatchBay.data) {
      ...
      const updated = await db.updateData(purchaseorderitem, { received: receivedQty, status }, { id })
      if (updated[0]) {
        res.send({ updated: updated[0].toString(), status, receivedQty, dispatchBay, qtyUpdated, customer })
      } else {
        res.send({ updated: updated[0].toString(), dispatchBay, customer, qtyUpdated })
      }
    } else {
  ...
  updateOrderStatus: async (req, res) => {
    ...
    const data = await db.getData(purchaseorderitem, { purchaseorderId: Number(id), shopId })
    let checker = data => data.every(item => item.status === 'full');
    console.log({ data, isTrue: checker(data) });
    let status = await db.getSingleField(purchaseorder, 'status', { id, shopId });
    if (checker(data)) {
      status = 'comp'
    }
  ...
  updatePO: async (req, res) => {
    ...
    const data = req.body
    console.log(data);
    const updated = await db.updateData(purchaseorder, data, { [Op.and]: { id, shopId } })
    if (updated[0]) {
  ...
  updatePoItem: async (req, res) => {
    ...
    if (data.hasOwnProperty('id')) {
      ...
      let qtys = []
      const variantId = await db.getSingleField(purchaseorderitem, "variantId", { id });
      const productIds = [{ status: 'new', id: data.variantId }, { status: 'old', id: variantId }]
      for (const i in productIds) {
        const shopData = await db.getSingleRecord(shop, { shopId });
        const response = await cda.getVariant(shopData.domain, productIds[i].id, shopData.accessToken)
        const qty = productIds[i].status === 'new' ? -Number(data.qty) : Number(data.qty)
        if (response.variant) {
          const { variant } = response
          const qtyUpdated = await cda.adjustQty(variant.inventory_item_id, qty, shopData)
    ...
    const updated = await db.updateData(purchaseorderitem, data, { [Op.and]: { id, shopId } })
    if (updated[0]) {
  ...
  AddProductToOpenPo: async (req, res) => {
    ...
    //#region update quantity in stock
    const shopData = await db.getSingleRecord(shop, { shopId });
    const productIds = [{ status: 'new', id: prod.variantId }, { status: 'old', id: oldVariantId }]
    let qtys = []
    for (const i in productIds) {
      const response = await cda.getVariant(shopData.domain, productIds[i].id, shopData.accessToken)
      const qty = productIds[i].status === 'new' ? -Number(prod.qty) : Number(prod.qty)
      if (response.variant) {
        const qtyUpdated = await cda.adjustQty(response.variant.inventory_item_id, qty, shopData)
    ...
    const vendorId = await db.getSingleField(vendor, 'id', { name: vendorName, shopId });
    const openOrder = await db.getSinglePurchaseOrders({ shopId, vendorId: vendorId, status: 'null', timeUntilCutoff: { [Op.gt]: ct } },)
    ...
    if (openOrder.id) {
      ...
      if (Object.keys(createdProduct).length > 0) {
        const statusUpdate = await db.updateData(purchaseorderitem, { status: 're' }, { id: oldProductId })

        const fields = { costValue: prod.costPerUnit * prod.qty, costValuePlusVat: (prod.costPerUnit + prod.costPlusTax) * prod.qty }
        const where = { id: prod.purchaseorderId }
        const incrementedCost = await db.incrementValue(purchaseorder, fields, where)
      }
      ...
      const po = {
        ...
        notes: '',
        status: 'null',
        costValuePlusVat: Number(((prod.costPerUnit + prod.costPlusTax) * prod.qty).toFixed(2)),
        submittedAt: null
      }
      console.log({ po });
  ...
  autoSubmit: async (req, res) => {
    ...
    const data = await db.getDataWithWhereAndInclude(purchaseorder, includeForOpen, whereForOpen, orderBy)
    console.log(data);
    if (data.length > 0) {
