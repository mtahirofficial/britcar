...
const { shop, webhook, order, vendor, cutoff, orderitem, purchaseorder, purchaseorderitem, sequelize } = dbModels;
...
const cda = {
    checkAccessTokenExpiry: async shop => {
        ...
        const errorText = '[API] Invalid API key or access token (unrecognized login or wrong password)'
        if (response.shop) {
            return true;
        } else {
            return false;
        ...
        if (activeOrders.length > 0) {
            ...
            for (const activeOrder of activeOrders) {
                for (const item of activeOrder.items) {
                    ...
                    const variantResponse = await cda.getVariant(domain, item.variantId, accessToken)
            ...
            for (const vendorName in vendors) {
                ...
                for (const item of vendors[vendorName].items) {
                    const orderNumber = await db.getSingleField(order, 'orderNumber', { orderId: item.orderId });
                    poCost = poCost + (Number(item.price) * item.quantity)
                    poCostPlusTax = poCostPlusTax + ((Number(item.price) + item.tax) * item.quantity)
                    const product = {
                        shopId,
                ...
                try {
                    const ct = new Date()
                    const vendorData = await db.getSingleRecord(vendor, { name: vendorName, shopId })
                    const openOrder = await db.getSinglePurchaseOrders({ shopId, vendorId: vendorData.id, status: 'null', timeUntilCutoff: { [Op.gt]: ct } },)
                    ...
                    if (openOrder.id) {
                        ...
                        for (const item of poItems[vendorName]) {
                            ...
                            const updated = db.updateData(order, { active: false }, { orderId: item.orderId })
                        }
                        const fields = {
                        ...
                        const incremented = await db.updateData(purchaseorder, fields, where)

                    } else {
                        cutoffs[vendorName] = await db.getVendorCutoffs(cutoff, { vendorId: vendorData.id })
    ...
    getDimensions: async (domain, accessToken, productId) => {
        ...
        if (metafields.length > 0) {
            for (const metafield of metafields) {
                if (metafield.key === 'width' || metafield.key === 'height' || metafield.key === 'length') {
    ...
    adjustQty: async (itemId, qty, shopData) => {
        const options = {
            ...
            data: { "location_id": shopData.locationId, "inventory_item_id": itemId, "available_adjustment": qty },
        };
    ...
    calculateCutOff: async (cutoffs, vendorName) => {
        ...
        let diff = 0;
        do {
            const cDay = current.toLocaleDateString('en-US', { weekday: 'short' }).toLowerCase()
            const cDate = (Number(current.getMonth()) + 1) + "/" + current.getDate() + "/" + current.getFullYear()
            const cDateForDb = current.getFullYear() + "-" + (Number(current.getMonth()) + 1) + "-" + current.getDate()

            let cutoff;
            let now = new Date()
            now = now.getTime()
            if (cutoffs[vendorName].hasOwnProperty(cDay)) {
                if (cutoffs[vendorName][cDay].hasOwnProperty('first') && cutoffs[vendorName][cDay].first) {
                    cutoff = Date.parse(cDate + " " + cutoffs[vendorName][cDay].first)
                    if (cutoff > now) {
                        console.log("first if");
                        diff = cutoff - now;
                        cutoffs[vendorName].time = cDateForDb + " " + cutoffs[vendorName][cDay].first;
                    }
                }
                if (cutoffs[vendorName][cDay].hasOwnProperty('last') && cutoffs[vendorName][cDay].last && diff <= 0) {
                    cutoff = Date.parse(cDate + " " + cutoffs[vendorName][cDay].last)
                    if (cutoff > now) {
                        console.log("last if");
                        diff = cutoff - now;
                        cutoffs[vendorName].time = cDateForDb + " " + cutoffs[vendorName][cDay].last;
                    }
                }
            }
