...
const productController = {
  ...
  updateProductMetafields: async (req, res) => {
    ...
    const productId = await db.getSingleField(orderitem, "productId", {
      variantId,
    });
    const metafield = {
      namespace: "custom",
      key: key,
      value: data[key],
      type: key === "weight" ? "number_decimal" : "single_line_text_field",
    };
    console.log("metafield", metafield);

    const metafieldResponse = await ShopifyController.setMetafields(
      productId,
      domain,
      accessToken,
      metafield,
    ...
    const dataForDb = {
      [key === "dimensions_cm" ? "dimensions" : key]: data[key],
    };
    if (metafieldResponse) {
      ...
      res.send({ isTrue: false, dataForDb, response: "Data not saved" });
    }
  },
  addMetafields: async (req, res) => {
    ...
    if (axiosResponse.metafields && axiosResponse.metafields.length > 0) {
      ...
      axiosResponse.metafields &&
      axiosResponse.metafields.length === 0
    ) {
      productFound = true;
    } else {
      productFound = false;
    ...
    if (productFound) {
      if (key === "weight") {
        ...
        response.weight = await cda.sendAxiosRequest({
          ...options,
          data: { metafield },
        });
      } else {
        response = { dimensions: {} };
        dataForDb = {
          [key]: `${data[key].width}x${data[key].height}x${data[key].length}`,
        };
        ...
        for (const dKey in data.dimensions) {
          ...
          response.dimensions[dKey] = await cda.sendAxiosRequest({
            ...options,
            data: { metafield },
          });
        }
      ...
      res.send({
        response: "Data updated in app but Product not found in stock!",
        dataForDb,
        isTrue: productFound,
      });
    }
  ...
  updateProductSku: async (req, res) => {
    ...
    console.log("metafieldResponse", metafieldResponse);
    if (metafieldResponse) {
      const b = await db.updateData(
        purchaseorderitem,
        { sku: metafieldResponse.value },
        { id: data.poItemId },
      );
      res.send({
        saved: "Data updated in Shopify and app!",
        bin: metafieldResponse.value,
        isTrue: true,
        variant: { sku: metafieldResponse.value },
      });
    } else {
      res.send({
        saved: "Data not updated.",
        bin: metafieldResponse.value,
        isTrue: false,
      });
    }
  ...
  deleteItem: async (req, res) => {
    ...
    const variantId = await db.getSingleField(purchaseorderitem, "variantId", {
      id: productId,
    });
    ...
    let qtyUpdated;
    if (response.variant) {
      qtyUpdated = await cda.adjustQty(
    ...
    const dbResponse = await db.deleteData(purchaseorderitem, {
      id: productId,
