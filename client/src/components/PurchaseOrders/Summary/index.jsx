...
const Summary = (props) => {
  ...
  const { receivedQty, itemStatus, purchaseOrders, checkBin } = useSelector(
      ...
      axios(options)
        .then(({ data }) => {
          ...
          if (data.updated) {
            ...
            for (const i in props.summaryOrders) {
              ...
              for (const j in order.purchaseorderitems) {
                if (order.purchaseorderitems[j].id === id) {
                  poIndex = i;
                  itemIndex = j;
                  props.summaryOrders[i].purchaseorderitems[j].received =
                    data.receivedQty;
                  props.summaryOrders[i].purchaseorderitems[j].status =
            ...
            setCompleted({
              dispatchBay: data.dispatchBay,
              customer: data.customer,
              order: props.summaryOrders[poIndex],
              item: selectedItem,
              active: true,
            });
            ...
            for (const orderIndex in pos) {
              if (pos[orderIndex].id === props.summaryOrders[poIndex].id) {
                ...
                for (const poItemIndex in order.purchaseorderitems) {
                  if (
                    order.purchaseorderitems[poItemIndex].id ===
                    props.summaryOrders[poIndex].purchaseorderitems[itemIndex]
                      .id
                  ) {
                  ...
                  if (
                    pos[orderIndex].purchaseorderitems[poItemIndex].id === id
                  ) {
                    pos[orderIndex].purchaseorderitems[poItemIndex].received =
                      data.receivedQty;
                  }
                }
            ...
            if (qty <= data.receivedQty) {
              props.summaryOrders[poIndex].purchaseorderitems.splice(
                itemIndex,
                1,
              );
            }
          ...
          <td className="td" style={{ textAlign: "center" }}>
            <Button
              ...
              onClick={() => {
                dispatch({
                  type: setCheckBin,
                  payload: { binError: "", bin: item.sku },
                });
                setStockReceive({
                  active: true,
                  product: { ...item, poNum: order.id, vendor: order.vendor },
                });
              }}
            >
              Receive Stock
    ...
    >
      ...
      <ModalComponent
        ...
        handleClose={() => {
          dispatch({
            type: setCheckBin,
            payload: { ...checkBin, binError: "" },
          });
          setStockReceive({ active: false, product: {} });
        }}
        primaryAction={{
          content: "Complete Line",
          disabled: Completing ? true : false,
          loading: Completing ? true : false,
          onAction: () => {
            if (
              StockReceive.product.qty <
                receivedQty.qtyToShow + receivedQty.received &&
              checkBin.bin === ""
            ) {
              dispatch({
                type: setCheckBin,
              ...
              updateReceivedQty(
                StockReceive.product.id,
                StockReceive.product.qty,
                receivedQty,
                StockReceive.product.variantId,
              );
              dispatch({
                type: setCheckBin,
                payload: { ...checkBin, binError: "" },
      ...
      <ModalComponent
        ...
        handleClose={() => {
          setCompleted({ dispatchBay: "", order: {}, item: {}, active: false });
        }}
        component={
          <OrderDetails
            order={Completed.order}
            item={Completed.item}
            dispatchBay={Completed.dispatchBay}
