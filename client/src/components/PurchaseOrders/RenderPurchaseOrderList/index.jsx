...
const RenderPurchaseOrderList = (props) => {
  ...
  const { moneyFormat, serverLink, shopId } = useSelector(
  ...
  const rows =
    purchaseOrders.length > 0
      ? purchaseOrders.map((order, index) => {
          ...
          const submittedDate =
            order.submittedAt !== null && order.submittedAt !== ""
              ? new Date(order.submittedAt)
              : "";
          order.submittedAt =
            submittedDate !== null && submittedDate !== ""
              ? `${submittedDate.toDateString()} ${submittedDate.getHours().toString().padStart(2, "0")}:${submittedDate.getMinutes().toString().padStart(2, "0")}`
              : "";
          let cost = 0;

          for (const product of order.purchaseorderitems) {
            cost += parseFloat(product.costPerUnit) * parseInt(product.qty);
          }
          cost = moneyFormat.replace("{{amount}}", cost.toFixed(2).toString());

          const orderCost = order.costValue
            ? moneyFormat.replace(
                "{{amount}}",
                order.costValue.toFixed(2).toString(),
              )
            : null;
          const costPlusTax = order.costValuePlusVat
            ? moneyFormat.replace(
                "{{amount}}",
                order.costValuePlusVat.toFixed(2).toString(),
              )
            : null;
          return (
            <tr className={`tr`} key={index}>
              ...
              <td className="td">{cost || orderCost}</td>
              ...
              <td className="td">
                {EditNotes[order.id] ? (
                  <TextField
                    ...
                    suffix={
                      NotesUpdating[order.id] ? (
                        <Spinner
                          accessibilityLabel="notes spinner"
                          size="small"
                        />
                      ) : null
                    }
