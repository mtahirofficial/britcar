...
const ReceiveStock = props => {
    ...
    const [Values, setValues] = useState({ qtyToShow: 0, bin: props.product.sku, weight: props.product.grams !== null && props.product.grams !== undefined && props.product.grams !== '' ? (props.product.grams / 1000).toString() : '', dimensions: props.product.dimensions })
    const [OldValues, setOldValues] = useState({ bin: '', weight: '', dimensions: '' })
    const [Error, setError] = useState({ bin: checkBin.binError, weight: '', dimensions: '' })
    const [ValuesUpdating, setValuesUpdating] = useState({ bin: '', weight: false, dimensions: false })

    useEffect(() => {
        dispatch({ type: setCheckBin, payload: { bin: props.product.sku, binError: '' } })
        dispatch({ type: receiveQty, payload: { qtyToShow: Values.qtyToShow, received: props.product.received } })
    }, [Values.qtyToShow])
    ...
    const handleChange = (key, value) => {
        if (key === 'bin' || key === 'qtyToShow') {
            ...
            let binError = ''
            if (key === 'qtyToShow') {
                if ((value + props.product.received) > props.product.qty && (Values.bin === '' || Values.bin === null)) {
    ...
    let checker = data => {
        if (data.length === 3) {
            let check = data.every(item => item.trim() !== '' && item.trim() !== 'undefined' && item.trim() !== undefined && !isNaN(item.trim()))
            return check
        } else {
            return false
        }
        ...
        <table className="receiveTable">
            <tbody className="receiveTBody">
                ...
                <tr>
                    ...
                    <th className="receiveTh"><span style={{ display: 'inline-block', maxWidth: '300px', whiteSpace: 'normal' }}>{props.product.description}</span></th>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <th className="receiveTh"><TextField type="number" value={Values.qtyToShow.toString()} onChange={value => handleChange('qtyToShow', Number(value))} min={0} /></th>
                </tr>
                <tr>
                    <td className="receiveTd">Notes</td>
                    <th className="receiveTh">{props.product.notes ? props.product.notes : '-'}</th>
                </tr>
                <tr>
                    <td className="receiveTd">Destination Bin</td>
                    <th className="receiveTh">
                        <TextField labelHidden focused={false} name="bin" label="Add destination bin" placeholder="Add destination bin" type="text"
                            value={Values.bin}
                            onFocus={() => {
                            ...
                            onBlur={({ target }) => {
                                if (Values.bin !== '') {
                                    if (Values.bin === OldValues.bin) {
                                        ...
                                        updateSku(props.product.variantId, { bin: Values.bin, poItemId: props.product.id }, target.name)
                                    }
                                }
                            }}
                            suffix={ValuesUpdating.bin ? <Spinner accessibilityLabel="bin spinner" size="small" /> : null}
                ...
                <tr>
                    ...
                    <th className="receiveTh">
                        <TextField labelHidden focused={false} name="weight" label="Add Weight" placeholder="Add Weight" type="text"
                            value={Values.weight}
                            onFocus={() => {
                                ...
                                if (Values.weight !== '' && Values.weight !== null) {
                                    if (Values.weight === OldValues.weight) {
                                        ...
                                        if (!isNaN(Values.weight)) {
                                            ...
                                            addMetafields(props.product.variantId, props.product.id, { weight: Number(Values.weight) }, target.name)
                                        } else {
                                            setError({ ...Error, [target.name]: 'Only Numbers are allowed.' })
                                        }
                                    }
                                }
                            }}
                            suffix={ValuesUpdating.weight ? <Spinner accessibilityLabel="weight spinner" size="small" /> : 'kgs'}
                            // helpText="In kgs."
                            error={Error.weight}
                        />
                    </th>
                ...
                <tr>
                    ...
                    <th className="receiveTh">
                        <TextField labelHidden focused={false} name="dimensions" label="Add Dimensions" placeholder="Add Dimensions" type="text"
                            ...
                            onBlur={({ target }) => {
                                if (Values.dimensions !== '' && Values.dimensions !== null) {
                                    ...
                                    if (Values.dimensions === OldValues.dimensions) {
                                        ...
                                        setError({ ...Error, [target.name]: <span>Dimensions must be added like <i>5</i>x<i>5</i>x<i>5</i></span> })
                                    } else {
                                        setError({ ...Error, [target.name]: '' })
                                        addMetafields(props.product.variantId, props.product.id, {
                                            dimensions: {
                            ...
                            suffix={ValuesUpdating.dimensions ? <Spinner accessibilityLabel="dimensions spinner" size="small" /> : null}
                            helpText={<span><i>width</i> x <i>height</i> x <i>length</i> | e.g. <i>5</i> x <i>5</i> x <i>5</i></span>}
                            error={Error.dimensions}
                        />
                    </th>
                </tr>
                <tr>
                    <td className="receiveTd" style={{ border: 'none' }}>Image</td>
                    <th className="receiveTh" style={{ border: 'none', textAlign: 'center' }}>
                        {
                            props.product.image ? <img src={props.product.image} alt="alt text" /> : 'Please take a picture of product'
                        }
                    </th>
                </tr>
            </tbody>
