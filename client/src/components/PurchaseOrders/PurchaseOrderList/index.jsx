...
const PurchaseOrderList = props => {
    ...
    const { serverLink, shopDomain } = useSelector(state => state.ConfigReducer)
    const { purchaseOrders, fetchingPos } = useSelector(state => state.POReducer)
            ...
            axios(options)
                .then(response => {
                    ...
                    if (response.data.createdOrders) {
                        ...
                        dispatch({ type: savePurchaseOrders, payload: response.data.createdOrders.concat(purchaseOrders) })
    ...
    // }
    return fetchingPos ? <Loader style={orderLoadingStyle} /> : (purchaseOrders.length > 0 ? <Page
        fullWidth
    ...
    >
        ...
        {
            message !== "" ? <Toast content={message} duration={2000} onDismiss={() => setMessage("")} /> : null
        }
    </Page > : <Page><EmptyList OrderGenerating={OrderGenerating} generatePurchaseOrders={generatePurchaseOrders} message={message} setMessage={setMessage} /></Page>)
}
