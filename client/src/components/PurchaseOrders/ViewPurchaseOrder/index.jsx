...
import { Page, Card, DataTable, Heading, DisplayText, Icon, TextField, Toast, Spinner } from '@shopify/polaris';
...
const ViewPurchaseOrder = props => {
    ...
    const { viewOrder, viewProduct, updateOrder, savePurchaseOrders } = actionTypes
    const dispatch = useDispatch()
    const { serverLink, moneyFormat, shopId } = useSelector(state => state.ConfigReducer)
    const { purchaseOrders, order, product, itemStatus, replacedProduct, poStatus } = useSelector(state => state.POReducer)

    const [OrderSubmitting, setOrderSubmitting] = useState(false)
    const [DownloadingCSV, setDownloadingCSV] = useState(false)
    const [Replace, setReplace] = useState(false)
    const [Delete, setDelete] = useState({ deleting: false, isConfirmed: false, orderId: '', productId: '', qty: '' })
                        ...
                        if (orders[i].id === orderId) {
                            for (const j in orders[i].purchaseorderitems) {
                                if (orders[i].purchaseorderitems[j].id === id) {
                                    ...
                                    setOldNotes({ ...OldNotes, [orders[i].purchaseorderitems[j].id]: requestData.notes })
                                }
                            }
                        }
                    }
    ...
    const replaceProduct = async (newProduct, oldProduct, order) => {
        ...
        if (ifConnected) {
            ...
            const prod = {
                ...
                id: oldProduct.id,
                image: newProduct.product.featuredImage ? newProduct.product.featuredImage.src : null,
    ...
    const submitOrder = date => {
        ...
        dispatch({ type: updateOrder, payload: { key: 'submittedAt', value: submittedAt } })
        const pos = purchaseOrders
        for (const po of pos) {
            if (po.id === order.id) {
    ...
    const deleteItem = (orderId, productId, qty) => {
        ...
        if (ifConnected) {
            ...
            axios(options)
                .then(({ data }) => {
                    setDelete({ deleting: false, isConfirmed: false, orderId: '', productId: '', qty: '' })
                    if (data.dbResponse) {
                        const orders = purchaseOrders
                        for (const i in orders) {
                            if (orders[i].id === orderId) {
    ...
    if (Object.keys(order).length > 0) {
        ...
        const cost = moneyFormat.replace("{{amount}}", order.costValue.toFixed(2).toString())
        ...
        const productDataRows = products.map((product, index) => {
            ...
            const actionsList = [
                ...
                },
                {
                    ...
                    destructive: true,
                    onAction: () => {
                        setDelete({ ...Delete, isConfirmed: true, orderId: order.id, productId: product.id, qty: product.qty })
                    }
                }
            ]
            return [
                product.barcode, <span style={{ display: 'inline-block', maxWidth: '300px', whiteSpace: 'normal' }}>{product.description}</span>, product.orderNumber, pCost, product.qty, product.received,
                <span>{
                    EditNotes[product.id] ? <TextField labelHidden focused={true} name="notes" label="Add Notes" placeholder="Add Notes" type="text"
                        ...
                        onBlur={() => {
                            if (!Notes[product.id] && !OldNotes[product.id]) {
                                ...
                                updateProductNotes(product.id, { notes: Notes[product.id] }, order.id)
                            }
                        }}
                        suffix={NotesUpdating[product.id] ? <Spinner accessibilityLabel="notes spinner" size="small" /> : null}
                    /> : <span style={{ display: 'inline-block', maxWidth: '300px', whiteSpace: 'normal' }} onClick={() => handleEdit(product.id, true)}>{product.notes}</span>
                }</span>,
                itemStatus[product.status], <ActionsList actionsList={actionsList} />
            ]
        })
        ...
        return <Page fullWidth>
            <Card
                ...
                actions={[
                    {
                        content: <Link to="/orders"><Icon source={CircleLeftMajor} color="base" /></Link>, onAction: () => dispatch({ type: viewOrder, payload: {} })
                    },
                    {
                        ...
                        onAction: () => {
                            ...
                            axios(options)
                                ...
                                .catch(error => {
                                    setOrderSubmitting(false)
                                    setError("Order is not submitted yet please try again")
                                    console.log("error", error)
                                })
                        }
                    },
                    {
                        content: <a href={`${serverLink}/files/generatePDF/${order.id}`} target="_blank" rel="noopener noreferrer">Download PDF</a>
                    },
                    {
                        content: DownloadingCSV ? null : <CSVLink
                            data={csvList}
                            headers={csvHeaders}
            ...
            >
                ...
                <Card.Section>
                    <DataTable
                        ...
                        headings={[<b>Code</b>, <b>Name</b>, <b>For Orders</b>, <b>Line Cost</b>, <b>Qty</b>, <b>Received</b>, <b>Notes</b>, <b>Status</b>, <b>Actions</b>]}
                        rows={productDataRows}
                    />
                </Card.Section>
            </Card>
            <ModalComponent large={false}
            ...
            <DeleteWarning
                ...
                status="warning"
                no={() => setDelete({ ...Delete, isConfirmed: false, orderId: '', productId: '', qty: '' })}
                yes={() => deleteItem(Delete.orderId, Delete.productId, Delete.qty)}
                yesLoading={Delete.deleting}
            />
            {
                error !== "" ? <Toast content={error} duration={2000} error={true} onDismiss={() => setError("")} /> : null
            }

        </Page >
    } else {
        return <DisplayText>No Order Selected! Go to <Link to="/orders">Orders</Link></DisplayText>
    }
}
