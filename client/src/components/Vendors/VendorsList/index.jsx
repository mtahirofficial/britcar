...
function VendorsList() {
  ...
  const { ConfigReducer, SupplierReducer, POReducer } = useSelector(
    (state) => state,
  );
    ...
    return (
      <tr key={i} className="tr">
        ...
        <td className="td" style={{ textAlign: "center" }}>
          <Button outline onClick={() => handleOpen(supplier)}>
            <Icon source={EditMinor} color="base" />
          </Button>
          <Button
            outline
            onClick={() => fetchSupplierBaseProducts(supplier.id)}
          >
            {FetchingProducts[supplier.id] ? (
              <Loader style={fetchingStyle} />
            ) : (
  ...
  return summaryOrders.length > 0 ? (
    <Summary summaryOrders={summaryOrders} />
  ) : (
    <Page
      fullWidth
    ...
    >
      ...
      <ModalComponent
        large={false}
        modalTitle={
          SupplierEdit.data.id === "" ? "New Supplier" : "Edit Supplier"
        }
        active={SupplierEdit.active}
        handleClose={handleClose}
        primaryAction={{
          content:
            SupplierEdit.data.id === "" ? "Save Supplier" : "Update Supplier",
          ...
          onAction: () => {
            ...
            const replyEmail =
              supplier.replyEmail !== "" && supplier.replyEmail !== null;
            const address =
